...
class TestDataUtils(unittest.TestCase):
    ...
    def test_encode_data(self):
        ...
        tensorIds, tensorAttn = data_utils.encode_data(self.dataset, self.tokenizer, self.max_seq_len)

        self.assertEqual(list(tensorIds.shape),[len(self.dataset), self.max_seq_len])
        self.assertEqual(list(tensorAttn.shape), [len(self.dataset), self.max_seq_len])
        self.assertEqual(tensorIds[0].dtype, torch.long)
        self.assertEqual(tensorAttn[0].dtype, torch.long)
    ...
    def test_extract_labels(self):
        ...
        lis = [1 if x else 0 for x in self.dataset["label"]]
        self.assertEqual(labels, lis)
